...
class BasketballMatchController extends AdminController
{
    public function index(Content $content)
    {
        $content->header('篮球比赛列表');
        $content->title('篮球比赛列表');
        $content->breadcrumb(['text' => '篮球比赛列表', 'url' => '/basketball/matchList']);
        $content->description('篮球比赛列表');
    ...
    {
        $grid = new Grid(new SportsBasketballMatchModel());
        $grid->model()->where(['is_deleted' => SportsBasketballMatchModel::DELETED_NO])->orderByDesc('match_time');
        ...
        $grid->filter(function($filter){
            ...
            $filter->column(1/2, function ($filter) {
                $filter->where(function ($query) {
                    $ids = SportsBasketballSeasonModel::where('year', 'LIKE', "%{$this->input}%")->pluck('id');
                    $query->whereIn('season_id', $ids);
                }, '赛季');
            });
            $filter->column(1/2, function ($filter) {
                $filter->where(function ($query) {
                    $ids = SportsBasketballCompetitionModel
            ...
            $filter->column(1/2, function ($filter) {
                $filter->where(function ($query) {
                    if ($this->input == "playing") {
                        $query->whereIn('status_id', SportsBasketballMatchModel::$playingStatusMap);
                    } else {
                        $query->where('status_id', $this->input);
                    }
                }, '状态')->select(SportsBasketballMatchModel::$statusAdminMap);
            });
            $filter->column(1/2, function ($filter) {
        ...
        $grid->column('season_id', __('admin.season'))->display(function () {
            $row = SportsBasketballSeasonModel::where(['id' => $this->season_id])->first();
            return $row->year ?? '--';
        });
        $grid->column('competition_id', __('admin.competition'))->display(function () {
            $row = SportsBasketballCompetitionModel::where(['id' => $this->competition_id])->first();
            return $row->name_zh ?? '--';
        });
        $grid->column('home_team_id', __('admin.home_team'))->display(function () {
            $row = SportsBasketballTeamModel::where(['id' => $this->home_team_id])->first();
        ...
        $grid->column('away_team_id', __('admin.away_team'))->display(function () {
            $row = SportsBasketballTeamModel::where(['id' => $this->away_team_id])->first();
            if (!empty($row->name_zh)) $text = $row->name_zh;
            if (!empty($row->logo) && !empty($row->name_zh))
                $text = "<img width='15' height='15' src='{$row->logo}'>{$row->name_zh}";
            return $text ?? '--';
        ...
        $grid->column('status_id', __('admin.match_status'))->display(function () {
            return SportsBasketballMatchModel::$statusMap[$this->status_id] ?? '--';
        });
        ...
        $grid->column('neutral', __('admin.neutral'))->display(function () {
            return SportsBasketballMatchModel::$neutralMap[$this->neutral] ?? '--';
        });
        $grid->column('live_url', __('admin.live_url'))->display(function () {
            $btn = $btn1 = $btn2 = $btn3 = '';
            if (in_array($this->status_id, array_merge([SportsBasketballMatchModel::STATUS_NOT_START], SportsBasketballMatchModel::$playingStatusMap)))
            {
                $func = function ($url) {
...
//        $grid->column('updated_at', __('admin.updated_at'));
    ...
    {
        $show = new Show(SportsBasketballMatchModel::findOrFail($id));
...
//        $show->field('updated_at', __('admin.updated_at'));
    ...
    {
        $form = new Form(new SportsBasketballMatchModel);
