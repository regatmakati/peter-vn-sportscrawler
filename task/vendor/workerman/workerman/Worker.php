...
class Worker
{
    ...
    protected static function parseCommand()
    {
        ...
        $master_is_alive = $master_pid && \posix_kill($master_pid, 0) && \getmypid() !== $master_pid;
    ...
    protected static function writeStatisticsToStatusFile()
    {
        ...
        /** @var \Workerman\Worker $worker */
        $worker            = current(static::$_workers);
        $worker_status_str = \getmypid() . "\t" . \str_pad(round(memory_get_usage(true) / (1024 * 1024), 2) . "M", 7)
    ...
    public static function checkErrors()
    {
        if (static::STATUS_SHUTDOWN !== static::$_status) {
            $error_msg = static::$_OS === \OS_TYPE_LINUX ? 'Worker['. \getmypid() .'] process terminated' : 'Worker process terminated';
