...
abstract class AbstractVideo extends Audio
{
    ...
    protected function buildCommand(FormatInterface $format, $outputPathfile)
    {
        ...
        if ($format instanceof VideoInterface) {
            if (null !== $format->getVideoCodec()) {
                if (config('params.gpu.nvidia.switch')) {   //使用NVIDIA GPU转码
                    $filters->add(new SimpleFilter(array('-c:v', $format->getVideoCodec())));
                    $filters->add(new SimpleFilter(array('-preset', 'slow')));
                } else {
    ...
    {
        if (config('params.gpu.nvidia.switch')) {   //使用NVIDIA GPU转码)
//            $commandArr = array('-y', '-vsync', '0', '-hwaccel', 'cuvid', '-c:v', 'h264_cuvid', '-resize', "{$this->videoWidth}x{$this->videoHeight}", '-i', $this->pathfile);
            $commandArr = array('-y', '-vsync', '0', '-hwaccel', 'cuvid', '-resize', "{$this->videoWidth}x{$this->videoHeight}", '-i', $this->pathfile);
        } else {
            $commandArr = array('-y', '-i', $this->pathfile);
        }
        return $commandArr;
    }

    public function setVideoWidth(int $width)
    {
        $this->videoWidth = $width;
    }

    public function setVideoHeight(int $height)
    {
